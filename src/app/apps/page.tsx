...
const sizeClasses: Record<string, string> = { "1x1": "bento-1x1", "2x1": "bento-2x1", "2x2": "bento-2x2" };
...
export default function AppsPage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <BentoGrid>
        {apps.map((app) => (
          <button key={app.id} onClick={() => setSelectedApp(app)} className={`bento-card cursor-pointer text-left ${sizeClasses[app.size]}`}>
            ...
            <div className="relative z-10 flex h-full flex-col">
              ...
              <h3 className="text-[15px] font-semibold sm:text-base" style={{ fontFamily: "'Space Grotesk', 'Inter', system-ui, sans-serif", color: "var(--color-text)" }}>{app.title}</h3>
              <p className="mt-1.5 text-[13px] leading-relaxed sm:text-sm" style={{ color: "var(--color-text-muted)" }}>{app.description}</p>
